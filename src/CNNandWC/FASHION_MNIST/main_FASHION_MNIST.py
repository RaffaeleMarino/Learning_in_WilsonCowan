...
(flat_train, y_train), (flat_test, y_test)= DM.load_Fashion_MNIST_dataset(perturb=noise)#
my_test=y_test
my_train=y_train

(x_train, y_train_cifar), (x_test, y_test_cifar) = tf.keras.datasets.fashion_mnist.load_data()
...
A=custom_model2.myA.numpy()
L=custom_model2.diag.numpy()
Phi=custom_model2.base.numpy()
np.savetxt('matrixAinter.txt', A)
np.savetxt('matrixLinter.txt', L)
np.savetxt('matrixPhiinter.txt', Phi)
